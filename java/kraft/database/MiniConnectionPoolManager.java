 ...
 */
final class MiniConnectionPoolManager {
	...
	private final LinkedList<PooledConnection> recycledConnections; // list of
	// inactive
	// PooledConnections
	...
	public synchronized void dispose() throws SQLException {
		...
		while (!recycledConnections.isEmpty()) {
			final PooledConnection pconn = recycledConnections.remove();
			try {
				pconn.close();
	...
	private Connection getConnection2(final long localTimeoutMs) throws SQLException {
		...
		try {
			if (!semaphore.tryAcquire(localTimeoutMs, TimeUnit.MILLISECONDS)) {
		...
		boolean ok = false;
		try {
			final Connection conn = getConnection3();
			ok = true;
			return conn;
		} finally {
			if (!ok) {
	...
	public Connection getValidConnection() {
		...
		final long timeoutTime = time + timeoutMs;
		int triesWithoutDelay = getInactiveConnections() + 1;
		while (true) {
			final Connection conn = getValidConnection2(time, timeoutTime);
			if (conn != null) {
				return conn;
			}
			triesWithoutDelay--;
	...
	private Connection getValidConnection2(final long time, final long timeoutTime) {
		...
		try {
			connection = getConnection2(rtime);
		} catch (final SQLException e) {
			return null;
		}
		rtime = timeoutTime - System.currentTimeMillis();
		final int rtimeSecs = Math.max(1, (int) ((rtime + 999) / 1000));
		try {
			if (connection.isValid(rtimeSecs)) {
				return connection;
			}
		} catch (final SQLException e) {
			//
	...
	private void log(final String msg) {
		final String s = "MiniConnectionPoolManager: " + msg;
		try {
			if (logWriter == null) {
